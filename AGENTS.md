...
| `plugins/sheets/` | Google Sheets mirror (opt-in plugin) — reconciles `data/applications.md` into a Google Sheet you own. One argument-free command serves both sync modes; scope comes from its cursor at `data/sheets-sync-state.json` (absent = full sync). `npm run sheets:sync:dry` previews, `npm run sheets:sync` writes. Never writes outside columns A:H, never inserts/deletes/sorts rows — see `plans/08-31-26_google-sheets-sync.md` |
