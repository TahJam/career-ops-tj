...
If yes → run `node update-system.mjs apply` (merges upstream via git; auto-resolves `.update-exclude` conflicts, stops and reports anything else for manual resolution — see `modes/_custom.md`). If no → run `node update-system.mjs dismiss`. Every other status (`up-to-date`, `dismissed`, `offline`, `no-remote-version` — checker reached GitHub but neither VERSION nor the latest release tag parsed as semver) → say nothing. The user can force a check anytime ("check for updates" / "update career-ops"); rollback: `node update-system.mjs rollback`.
...
If the user accepts, use the `/loop` or `/schedule` skill (if available) to set up a recurring `/career-ops scan`. If those aren't available, point them to [docs/AUTOMATION.md](docs/AUTOMATION.md) for copy-paste cron / launchd / Windows Task Scheduler recipes, or remind them to run `/career-ops scan` periodically.
