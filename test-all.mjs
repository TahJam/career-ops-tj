...
if (!existsSync(join(ROOT, 'modes/zh/_shared.md'))) {
  warn('Chinese modes include company-type compensation reliability checks (skipped: modes/zh/ removed in this fork)');
} else {
...
// MCP fallback — so the flag actually reaches the JD paths, not just scan/pipeline.
{
  ...
  if (langPipelines.length === 0) {
    warn('language pipeline mirror extractor wiring (#1449 Phase 2b) skipped: no modes/{lang}/pipeline.md present (removed in this fork)');
  } else if (langMissing.length === 0) {
    pass(`all ${langPipelines.length} language pipeline mirrors wire the opt-in extractor (#1449 Phase 2b)`);
  } else {
    fail(`language pipeline mirrors missing extractor wiring: ${langMissing.join(', ')}`);
  }
}
...
try {
  ...
  try {
    ...
    if (clobberResult === null) {
      ...
      // This fork's tier-3 is exact-match only (see the exact-match
      // regression test above) — a reworded repost title no longer matches
      // at all, so there is no fuzzy tier left to bump-and-preserve. It is
      // added as its own row and the original Applied row is left completely
      // untouched, which is the safer of the two outcomes: a false
      // "not a duplicate" here only produces an extra row a human can dedupe
      // later, unlike a false "duplicate" silently destroying data.
      const acmeRows = merged.split('\n').filter(l => l.includes('Observability'));
      if (acmeRows.length === 2
          && acmeRows.some(r => r.includes('Senior Platform Engineer, Observability') && r.includes('3.9/5') && r.includes('Applied'))
          && acmeRows.some(r => r.includes('Sr Platform Engineer, Observability (Remote)') && r.includes('4.2/5'))) {
        pass('reworded repost added as its own row; original Applied row left untouched (exact-match tier-3)');
      } else {
        fail(`exact-match tier-3 behavior broken: ${acmeRows.length} Observability rows: ${acmeRows.join(' // ')}`);
      }
    }
  } finally {
