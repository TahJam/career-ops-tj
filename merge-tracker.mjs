...
// Sync PDF flags (idempotent; uses its own lock/transaction). Forward the
// same isolated PDF_INDEX_FILE this run resolved above — otherwise the child
// process falls back to its own default (join(CAREER_OPS, 'data',
// 'pdf-index.tsv')), which is the REAL repo's manifest even when
// CAREER_OPS_TRACKER points at an isolated test fixture (#1429 follow-up).
if (!DRY_RUN) {
  try {
    execFileSync('node', [join(CAREER_OPS, 'sync-pdf-flags.mjs')], {
      stdio: 'inherit',
      env: { ...process.env, CAREER_OPS_PDF_INDEX: PDF_INDEX_FILE },
    });
  } catch (e) {
